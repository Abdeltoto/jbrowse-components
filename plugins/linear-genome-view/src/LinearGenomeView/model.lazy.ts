...
export type LinearGenomeViewModel = Instance<ReturnType<typeof enhance>>

export function enhance(base: LinearGenomeViewBaseModel) {
  return base
    .volatile(() => ({
      /**
       * #volatile
    ...
    .views(self => {
      ...
      return {
        ...
        /**
         * #getter
         * Merges dynamic block regions by regionNumber into a single region
         * per displayed region. Returns raw (fractional) coordinates.
         * For data fetching, use roundedVisibleRegions instead.
         */
        ...
        /**
         * #getter
         * Visible regions with pre-fetch padding, integer-rounded coordinates,
         * and clamped to assembly bounds. Suitable for passing to data
         * adapters via RPC.
         */
        get roundedVisibleRegions() {
          const bufferBp = this.width * this.bpPerPx * 0.5
          return this.mergedVisibleRegions.map(vr => {
            const dr = this.displayedRegions[vr.regionNumber]
            return {
              region: {
                refName: vr.refName,
                assemblyName: vr.assemblyName,
                start: Math.floor(
                  Math.max(dr?.start ?? 0, vr.start - bufferBp),
                ),
                end: Math.ceil(
                  Math.min(dr?.end ?? vr.end, vr.end + bufferBp),
                ),
              },
